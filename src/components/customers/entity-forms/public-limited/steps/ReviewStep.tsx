...
import { PublicLimitedData } from '../../types/public-limited'

interface ReviewStepProps { data: PublicLimitedData; onEdit: (step: number) => void }
...
export default function ReviewStep({ data, onEdit }: ReviewStepProps) {
  ...
  const kmps = data.kmps || []

  const isCompanyComplete = !!(data.company_name && data.cin && data.date_of_incorporation && data.company_pan && data.roc_office && data.listed_status)
  const isDirectorsComplete = directors.length >= 3 && directors.every(d => d.full_name && d.din && d.pan_number)
  const isShareholdersComplete = true // Optional section
  const isAddressComplete = !!(data.registered_address_line1 && data.registered_city && data.registered_state && data.registered_pincode)
  const isDocsComplete = !!(data.certificate_of_incorporation_url && data.moa_url && data.aoa_url && data.company_pan_url && data.board_resolution_url && data.bank_statement_url)
  const isAllComplete = isCompanyComplete && isDirectorsComplete && isShareholdersComplete && isAddressComplete && isDocsComplete

  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold text-white mb-2">Review & Submit</h2><p className="text-gray-400">Review your Public Limited Company details</p></div>
      ...
      <div className="space-y-4">
        <ReviewCard title="Company Details" icon={<Building className={`w-5 h-5 ${isCompanyComplete ? 'text-orange-400' : 'text-yellow-400'}`} />} step={1} onEdit={onEdit} isComplete={isCompanyComplete}>
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div><span className="text-gray-500">Listed Status</span><p className="text-white">{data.listed_status || '-'}</p></div>
          ...
          <div className="space-y-2">
            {directors.map((d, i) => (
              <div key={d.id} className="flex items-center gap-3 p-2 bg-gray-800 rounded-lg">
                ...
                {d.director_type === 'INDEPENDENT_DIRECTOR' && <span className="text-xs bg-orange-500/20 text-orange-400 px-2 py-0.5 rounded">ID</span>}
              </div>
            ))}
          </div>
        </ReviewCard>

        <ReviewCard title={`Major Shareholders (${shareholders.length})`} icon={<PieChart className={`w-5 h-5 ${isShareholdersComplete ? 'text-orange-400' : 'text-yellow-400'}`} />} step={3} onEdit={onEdit} isComplete={isShareholdersComplete}>
          <div className="space-y-2">
            {shareholders.length > 0 ? shareholders.slice(0, 5).map((s, i) => (
            ...
            )) : <p className="text-gray-500 text-sm">No shareholders added (optional)</p>}
        ...
        <ReviewCard title="Documents" icon={<FileCheck className={`w-5 h-5 ${isDocsComplete ? 'text-orange-400' : 'text-yellow-400'}`} />} step={5} onEdit={onEdit} isComplete={isDocsComplete}>
          <div className="grid grid-cols-2 gap-2 text-sm">
            {[{ l: 'COI', v: data.certificate_of_incorporation_url }, { l: 'MOA', v: data.moa_url }, { l: 'AOA', v: data.aoa_url }, { l: 'Company PAN', v: data.company_pan_url }, { l: 'Board Resolution', v: data.board_resolution_url }, { l: 'Bank Statement', v: data.bank_statement_url }, { l: 'Audited Financials', v: data.audited_financials_url }].map(d => (
