...
function DocCard({ title, description, required, value, error, onUpload, onRemove }: DocCardProps) {
  ...
  const handleFile = async (file: File) => {
    if (!file) return
    try { setUploading(true); await new Promise(r => setTimeout(r, 1000)); onUpload(`https://storage.example.com/society/${Date.now()}-${file.name}`) }
...
export default function DocumentsStep({ data, errors, onUpdate }: SocietyStepProps) {
  const docs = [
    { key: 'registration_certificate_url', title: 'Registration Certificate', description: 'Certificate from Registrar of Societies', required: true },
    { key: 'memorandum_of_association_url', title: 'Memorandum of Association', description: 'MOA document', required: true },
    { key: 'rules_regulations_url', title: 'Rules & Regulations', description: 'Society bylaws', required: false },
    { key: 'society_pan_url', title: 'Society PAN Card', description: 'PAN card of the society', required: true },
    { key: 'resolution_url', title: 'Resolution', description: 'Latest governing body resolution', required: false },
    { key: 'certificate_12a_url', title: '12A Certificate', description: 'If applicable', required: false },
    { key: 'certificate_80g_url', title: '80G Certificate', description: 'If applicable', required: false },
    { key: 'bank_statement_url', title: 'Bank Statement', description: 'Last 6 months', required: true },
    { key: 'address_proof_url', title: 'Address Proof', description: 'Utility bill or rent agreement', required: false },
    { key: 'annual_report_url', title: 'Annual Report', description: 'Latest financial year', required: false },
    { key: 'member_list_url', title: 'Member List', description: 'Current members list', required: false }
  ]

  return (
    <div className="space-y-8">
      <div><h2 className="text-2xl font-bold text-white mb-2">Documents & KYC</h2><p className="text-gray-400">Upload required society documents</p></div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div><h3 className="text-lg font-semibold text-white">Society Documents</h3></div>
      </div>
      ...
      <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
        <div className="flex items-start gap-3"><AlertCircle className="w-5 h-5 text-yellow-400 flex-shrink-0" /><div><p className="text-yellow-400 font-medium">Member Documents</p><p className="text-yellow-300 text-sm">Individual governing body member documents can be uploaded in the Governing Body section.</p></div></div>
      </div>
    </div>
  )
}
