...
import { Users, Grid3x3, List, Percent, Info, Shield } from 'lucide-react'
...
export default function PartnersStepEnhanced({ data, errors, onUpdate }: LLPStepProps) {
    ...
    const initialMembers = Array.from({ length: numPartners }, (_, i) => {
      ...
      if (existingPartner && existingPartner.full_name) {
        return {
          ...
          designation: existingPartner.dpin_din,
          capital_contribution_percent: existingPartner.profit_sharing_percent,
          profit_sharing_percent: existingPartner.profit_sharing_percent,
          photo_url: '',
          is_signatory: existingPartner.is_authorized_signatory || false,
  ...
  const handleSaveMember = (member: MemberData) => {
    ...
    // Convert to LLP format and update parent
    const partners = data.partners || []
    const newPartners = [...partners]
  ...
  const designatedPartners = filledMembers.filter(m => m.role === 'DESIGNATED_PARTNER')
  const hasMinDesignatedPartners = designatedPartners.length >= 2
    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-400">
          Add all partners including Designated Partners (minimum 2 Designated Partners required)
        </p>
      </div>
      ...
      <div className="max-w-md mx-auto p-5 bg-gray-800/50 border border-gray-700 rounded-xl">
        <label className="block text-sm font-medium text-gray-300 mb-3">
          How many partners in this LLP? <span className="text-red-400">*</span>
        </label>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className={`border rounded-xl p-4 ${
          hasMinDesignatedPartners ? 'bg-green-500/10 border-green-500/30' :
          designatedPartners.length > 0 ? 'bg-yellow-500/10 border-yellow-500/30' : 'bg-gray-800 border-gray-700'
        }`}>
          <div className="flex items-center gap-2 mb-1">
          ...
          <p className={`text-2xl font-bold ${
            hasMinDesignatedPartners ? 'text-green-400' :
            designatedPartners.length > 0 ? 'text-yellow-400' : 'text-white'
          }`}>
            {designatedPartners.length}
          </p>
      ...
      {/* Validation Warnings */}
      {!hasMinDesignatedPartners && filledMembers.length > 0 && (
        <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
          <div className="flex items-start gap-3">
            <Info className="w-5 h-5 text-yellow-400 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-yellow-400 font-medium">At least 2 Designated Partners required</p>
              <p className="text-sm text-gray-300 mt-1">
                An LLP must have at least 2 Designated Partners. Please designate at least 2 partners as Designated Partners.
              </p>
            </div>
          </div>
        </div>
