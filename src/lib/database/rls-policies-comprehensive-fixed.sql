...
BEGIN
    FOR r IN (SELECT tablename FROM pg_tables WHERE schemaname = 'public') LOOP
        ...
        -- Drop our new policies for clean re-application
        EXECUTE 'DROP POLICY IF EXISTS "profiles_select_own" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "profiles_select_staff" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "profiles_update_own" ON ' || quote_ident(r.tablename);
        EXECUTE 'DROP POLICY IF EXISTS "profiles_update_admin" ON ' || quote_ident(r.tablename);
    END LOOP;
END $$;
...
-- NOTE: Role changes prevented via application logic and triggers
...
-- UPDATE: Customers can update their own profile
-- NOTE: Critical fields (credit_score, credit_limit) protected via triggers
...
-- UPDATE: Partners can update their own profile
-- NOTE: Financial fields protected via triggers
...
-- UPDATE: Employees can update their own profile
-- NOTE: Permissions and salary protected via triggers
...
-- NOTE: Performance metrics protected via triggers
...
-- UPDATE: Staff only
-- NOTE: Core transaction data immutability enforced via triggers
...
-- UPDATE: Partners can update their referrals
-- NOTE: Commission fields protected via triggers
...
-- SECURITY NOTE: USING (true) acceptable for service_role as it already bypasses RLS
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '✅ COMPREHENSIVE RLS POLICIES APPLIED (FIXED)';
    RAISE NOTICE '========================================';
    ...
    RAISE NOTICE '  • Field-level protection (credit_score, salary, etc.)';
    RAISE NOTICE '    enforced via database triggers (see separate file)';
    RAISE NOTICE '  • Transaction immutability enforced via triggers';
    RAISE NOTICE '  • Apply trigger protection file after this script';
    RAISE NOTICE '========================================';
END $$;
