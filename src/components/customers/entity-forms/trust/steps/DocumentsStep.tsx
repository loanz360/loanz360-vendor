...
function DocCard({ title, description, required, value, error, onUpload, onRemove }: DocCardProps) {
  ...
  const handleFile = async (file: File) => {
    if (!file) return
    try { setUploading(true); await new Promise(r => setTimeout(r, 1000)); onUpload(`https://storage.example.com/trust/${Date.now()}-${file.name}`) }
...
export default function DocumentsStep({ data, errors, onUpdate }: TrustStepProps) {
  const docs = [
    { key: 'trust_deed_url', title: 'Trust Deed', description: 'Registered trust deed document', required: true },
    { key: 'registration_certificate_url', title: 'Registration Certificate', description: 'If registered under state law', required: false },
    { key: 'trust_pan_url', title: 'Trust PAN Card', description: 'PAN card of the trust', required: true },
  ...
  return (
    <div className="space-y-8">
      <div><h2 className="text-2xl font-bold text-white mb-2">Documents & KYC</h2><p className="text-gray-400">Upload required trust documents</p></div>
      <div className="flex items-center gap-3 mb-4">
        ...
        <div><h3 className="text-lg font-semibold text-white">Trust Documents</h3></div>
      </div>
      ...
      <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
        <div className="flex items-start gap-3"><AlertCircle className="w-5 h-5 text-yellow-400 flex-shrink-0" /><div><p className="text-yellow-400 font-medium">Trustee Documents</p><p className="text-yellow-300 text-sm">Individual trustee documents can be uploaded in the Trustees section.</p></div></div>
      </div>
    </div>
  )
}
