/**
 * API Route: BP Lead Phase 1 Submission
 * POST /api/partners/bp/leads/submit
 *
 * Handles Phase 1 loan application submission by BP with:
...
/** Document attachment included with a lead submission */
...
interface SubmitLeadRequest {
  ...
  // Lead quality score (calculated on client)
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const traceToken = generateTraceToken({
      ...
      partnerCode: partner.partner_id || 'BP-UNKNOWN',
    })

    // 10. Phase 1: NO BDE auto-assignment or lead quality scoring
    // These happen in Phase 2 after complete application is submitted
    // Lead goes to Unified CRM Pipeline with status NEW_UNASSIGNED
    ...
    notifyLeadCreated(
      ...
      undefined
    ).catch((err: unknown) => apiLogger.error('Lead notification error', err instanceof Error ? err : undefined))
    ...
    apiLogger.error('Submit lead error', error instanceof Error ? error : undefined)
    return NextResponse.json(
      {
        success: false,
