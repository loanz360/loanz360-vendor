  ...
  try {
    ...
    if (type === 'contacts') {
      ...
      data = (contacts || []).map((c) => ({
        ...
        status: c.status,
        cro_name: croNameMap.get(c.assigned_to_cro) || 'Unknown',
      ...
      data = (leads || []).map((l) => ({
        ...
        status: l.status,
        stage: l.stage,
        cro_name: croNameMap.get(l.cro_id) || 'Unknown',
      ...
      data = (deals || []).map((d) => ({
        ...
        status: d.status,
        stage: d.stage,
        cro_name: croNameMap.get(d.cro_id) || 'Unknown',
    ...
    logger.error('[CRO-TL Pipeline] Error', { requestId, error: error instanceof Error ? error.message : 'Unknown' })
