...
/**
 * GET /api/partners/bp/profile
 * Fetches BP partner profile
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (profileError && profileError.code !== 'PGRST116') {
      apiLogger.error('Error fetching BP profile', profileError)
      return NextResponse.json(
    ...
    // Return existing profile with null-safe defaults for string fields
    return NextResponse.json({
      success: true,
      profile: {
    ...
    apiLogger.error('Error in GET /api/partners/bp/profile', error)
    return NextResponse.json(
...
/**
 * PUT /api/partners/bp/profile
 * Updates BP partner profile
 */
export async function PUT(request: NextRequest) {
  try {
    ...
    if (error) {
      apiLogger.error('Error upserting BP profile', error)
      return NextResponse.json(
        {
          success: false,
    ...
    apiLogger.error('Error in PUT /api/partners/bp/profile', error)
    return NextResponse.json(
      { success: false, error: 'Internal server error' },
      { status: 500 }
    )
