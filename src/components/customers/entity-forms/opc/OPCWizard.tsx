...
import DirectorStepEnhanced from './steps/DirectorStepEnhanced'
import NomineeStep from './steps/NomineeStep'
import AddressStep from './steps/AddressStep'
import DocumentsStep from './steps/DocumentsStep'
import ReviewStep from './steps/ReviewStep'
import { OPCData, initialOPCData } from '../types/opc'

export interface OPCWizardProps {
  onComplete: () => void
  onBack?: () => void
  initialData?: Partial<OPCData>
  entityId?: string
...
const STEPS = [
  { id: 1, title: 'Company Details', icon: Building2, description: 'OPC information' },
  { id: 2, title: 'Director', icon: User, description: 'Sole director details' },
  { id: 3, title: 'Nominee', icon: UserPlus, description: 'Nominee director' },
...
export default function OPCWizard({
  onComplete,
  onBack,
  initialData,
  entityId,
  ...
  const [formData, setFormData] = useState<OPCData>({
    ...initialOPCData,
    ...initialData
  })
  ...
  const updateFormData = useCallback((updates: Partial<OPCData>) => {
  ...
  const validateStep = (_step: number): boolean => {
    ...
    switch (step) {
      ...
      case 2:
        if (!formData.director.full_name?.trim()) newErrors.director_name = 'Director name is required'
        if (!formData.director.din?.trim()) newErrors.director_din = 'DIN is required'
        if (!formData.director.pan_number?.trim()) newErrors.director_pan = 'Director PAN is required'
        else if (!/^[A-Z]{5}[0-9]{4}[A-Z]{1}$/.test(formData.director.pan_number)) newErrors.director_pan = 'Invalid PAN format'
        if (!formData.director.mobile?.trim()) newErrors.director_mobile = 'Mobile number is required'
        if (!formData.director.email?.trim()) newErrors.director_email = 'Email is required'
        break

      case 3:
        if (!formData.nominee.nominee_name?.trim()) newErrors.nominee_name = 'Nominee name is required'
        if (!formData.nominee.nominee_pan?.trim()) newErrors.nominee_pan = 'Nominee PAN is required'
        if (!formData.nominee.relationship) newErrors.nominee_relationship = 'Relationship is required'
        break
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await response.json()
      if (data.success) {
        // Auto-create customer profile for director
        const newEntityId = data.data?.entity?.id || entityId
        if (newEntityId && formData.director && formData.director.full_name && formData.director.mobile && formData.director.email && formData.director.pan_number) {
          toast.success('OPC created! Creating director profile...')
          ...
          try {
            const memberResponse = await fetch('/api/customers/entity-members/create-profile', {
              ...
              body: JSON.stringify({
                ...
                member_id: formData.director.id || `director-${Date.now()}`,
                member_data: {
                  name: formData.director.full_name,
                  mobile: formData.director.mobile,
                  email: formData.director.email,
                  pan: formData.director.pan_number,
                  aadhaar: formData.director.aadhaar_number,
                  role: 'DIRECTOR',
                  designation: formData.director.designation || 'Director',
            ...
            console.error('Error creating director profile:', err)
          }
        } else {
          toast.success('OPC profile submitted successfully!')
        }
        onComplete()
      } else {
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2: return <DirectorStepEnhanced data={formData} errors={errors} onUpdate={updateFormData} />
      case 3: return <NomineeStep data={formData} errors={errors} onUpdate={updateFormData} />
