...
/**
 * POST /api/notifications/ab-tests/[id]/start
...
export async function POST(
  ...
  try {
    ...
    if (existing.status !== 'draft' && existing.status !== 'paused') {
      return NextResponse.json(
        { success: false, error: `Cannot start a test with status "${existing.status}". Only draft or paused tests can be started.` },
        { status: 409 }
      )
    }

    // Validate the test has proper variants before starting
    const variants = existing.variants as Array<{ content?: string }>
    if (!variants || variants.length < 2) {
      return NextResponse.json(
        { success: false, error: 'Test must have at least 2 variants to start' },
        { status: 400 }
      )
    }
    ...
    // Only set start_date on first start (not when resuming from pause)
    ...
    return NextResponse.json({
      success: true,
      test,
      message: existing.status === 'draft' ? 'A/B test started successfully' : 'A/B test resumed successfully',
    })
  } catch (error) {
    apiLogger.error('Error starting A/B test', error)
    logApiError(error as Error, request, { action: 'start_ab_test' })
    return NextResponse.json(
