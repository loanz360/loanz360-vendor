...
interface DocumentUploadCardProps {
  title: string
  description: string
  fieldKey: keyof CooperativeStepProps['data']
  currentUrl: string
  isRequired?: boolean
...
export default function DocumentsStep({ data, errors, onUpdate }: CooperativeStepProps) {
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <DocumentUploadCard
          ...
          currentUrl={data.registration_certificate_url}
          isRequired={true}
          onUpload={(url) => onUpdate({ registration_certificate_url: url })}
          error={errors.registration_certificate_url}
        />

        <DocumentUploadCard
          title="Bye-Laws"
          description="Society bye-laws document"
          fieldKey="bye_laws_url"
          currentUrl={data.bye_laws_url}
          onUpload={(url) => onUpdate({ bye_laws_url: url })}
        />

        <DocumentUploadCard
          title="Society PAN Card"
          description="PAN card of the society"
          fieldKey="society_pan_url"
          currentUrl={data.society_pan_url}
          isRequired={true}
          onUpload={(url) => onUpdate({ society_pan_url: url })}
          error={errors.society_pan_url}
        />

        <DocumentUploadCard
          title="Board Resolution"
          description="Resolution for loan application"
          fieldKey="resolution_url"
          currentUrl={data.resolution_url}
          onUpload={(url) => onUpdate({ resolution_url: url })}
        />

        <DocumentUploadCard
          title="Bank Statement"
          description="Last 6 months bank statement"
          fieldKey="bank_statement_url"
        ...
        <DocumentUploadCard
          ...
          fieldKey="audit_report_url"
          currentUrl={data.audit_report_url}
          onUpload={(url) => onUpdate({ audit_report_url: url })}
        />

        <DocumentUploadCard
          title="Annual Return"
          description="Latest annual return filed"
          fieldKey="annual_return_url"
          currentUrl={data.annual_return_url}
          onUpload={(url) => onUpdate({ annual_return_url: url })}
        />

        <DocumentUploadCard
          title="Member List"
          description="List of all society members"
          fieldKey="member_list_url"
          currentUrl={data.member_list_url}
          onUpload={(url) => onUpdate({ member_list_url: url })}
        />
      </div>
