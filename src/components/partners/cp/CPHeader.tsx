...
export function CPHeader({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  const handleSignOut = async () => {
    // Always redirect to partners login page since this is the CP (Channel Partner) Header
          ...
          <div className="flex-1 lg:w-[80%] flex justify-between items-center pr-2 lg:pr-6">
            ...
            <div className="flex-1 hidden sm:block">
              <h1 className="text-xl font-bold font-poppins leading-tight">
                Welcome back, {user?.full_name || 'Channel Partner'}
              </h1>
              <p className="text-gray-400 text-base hidden md:block">
                Manage deals, track analytics, and access exclusive resources.
              </p>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              {/* Notifications */}
              <NotificationDropdown partnerType="cp" />
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  ...
                  <span className="text-base font-medium">{user?.full_name || 'Channel Partner'}</span>
                ...
                {showDropdown && (
                  <div className="absolute right-0 mt-2 w-56 bg-gray-900/95 backdrop-blur-lg rounded-lg shadow-xl border border-gray-700/50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-3 border-b border-gray-700/50">
                      <p className="text-white text-sm font-medium">{user?.full_name || 'Channel Partner'}</p>
                    ...
                    <div className="py-2">
                      <button
                        onClick={() => {
                          setShowDropdown(false)
                          router.push('/partners/cp/profile')
                        }}
                      ...
                      <button
                        onClick={() => {
                          setShowDropdown(false)
                          router.push('/partners/cp/notifications')
                        }}
                        className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors flex items-center space-x-2"
                      >
                      ...
                      <button
                        onClick={() => {
                          setShowDropdown(false)
                          router.push('/partners/cp/support-tickets')
                        }}
                        className="w-full px-4 py-2 text-left text-sm text-gray-300 hover:text-white hover:bg-gray-800/50 transition-colors flex items-center space-x-2"
                      >
