...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Fetch all call logs for the team in this period
    ...
    logs.forEach((l) => {
      outcomes[l.call_outcome] = (outcomes[l.call_outcome] || 0) + 1
    })
    ...
    const dailyTrend = Array.from(dailyMap.entries()).map(([date, data]) => ({
      date, calls: data.calls, connected: data.connected,
    }))
    ...
    const croBreakdown = Array.from(croMap.entries()).map(([croId, data]) => ({
      ...
      avgDuration: data.calls > 0 ? Math.round(data.duration / data.calls) : 0,
    ...
    logger.error('[CRO-TL Call Analytics] Error', { requestId, error: error instanceof Error ? error.message : 'Unknown' })
