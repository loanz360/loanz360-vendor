...
export function CPBanner() {
  const { user } = useAuth()

  // Get dynamic banner (changes on each login)
  const bannerContent = useMemo(() => {
    return getLoginBanner('CHANNEL_PARTNER')
  }, [])
        ...
        <div className="w-full flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-200 text-sm drop-shadow-lg">
              Welcome back, <span className="text-white font-semibold">{user?.full_name || user?.email?.split('@')[0] || 'Channel Partner'}</span>
              <span className="text-gray-400 mx-2">|</span>
              <span className="text-gray-300">Track deals, analyze performance, and unlock new opportunities</span>
            </p>
          </div>
          {bannerContent && (
