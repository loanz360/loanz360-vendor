/**
 * API Route: BA Dashboard Aggregated Data
 * GET /api/partners/ba/dashboard - Get aggregated dashboard data for BA
 *
 * Combines lead stats, commission summary, recent leads, contests,
 * and weekly goal into a single endpoint for the BA dashboard.
 *
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // 5. Calculate this month's stats from parallel query result
    ...
    // 9. Weekly goal (default target of 10 leads per week)
    const weeklyGoal = {
      target: 10,
    ...
    apiLogger.error('BA Dashboard error', error)
    return NextResponse.json(
      {
        success: false,
