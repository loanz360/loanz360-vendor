 ...
 * Includes both direct BP applications and team override applications
 *
 * GET: List BP payout applications pending verification
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (mode === 'history') {
      ...
      if (historyError) {
        logger.error('Error fetching BP application history:', { error: historyError })
    ...
    const tab = searchParams.get('tab') || 'all' // 'direct', 'team', 'all'
    const page = Math.max(1, parseInt(searchParams.get('page') || '1', 10))
    const limit = Math.min(100, Math.max(1, parseInt(searchParams.get('limit') || '20', 10)))

    // Query partner_payout_applications for BP
    let query = supabase
    ...
    // Filter by tab
    if (tab === 'direct') {
      query = query.eq('is_team_override', false)
    } else if (tab === 'team') {
      query = query.eq('is_team_override', true)
    }
    // 'all' = both direct and team overrides
    ...
    if (error) {
      ...
      return NextResponse.json({ success: false, error: 'Failed to fetch BP applications' }, { status: 500 })
    }
    ...
    if (partnerIds.length > 0) {
      ...
      if (partners) {
        partnerMap = Object.fromEntries(partners.map(p => [p.id, p.full_name || p.partner_id || 'BP Partner']))
      }
    }

    const enrichedApplications = (applications || []).map(app => ({
      ...app,
      partner_name: partnerMap[app.partner_id] || 'BP Partner',
    }))

    // Stats — filtered by tab to match what's shown in table
    const today = new Date().toISOString().split('T')[0]

    const buildStatsQuery = (baseStatus: string) => {
      let q = supabase.from('partner_payout_applications').select('id', { count: 'exact', head: true })
        .eq('partner_type', 'BP').eq('status', baseStatus)
      if (tab === 'direct') q = q.eq('is_team_override', false)
      else if (tab === 'team') q = q.eq('is_team_override', true)
      return q
    }
    ...
    const [pendingResult, inVerificationResult, verifiedTodayResult, pendingAmountResult, saApprovedResult, financeProcessingResult] = await Promise.all([
      ...
      buildStatsQuery('ACCOUNTS_VERIFICATION'),
      (() => {
        let q = supabase.from('partner_payout_applications').select('id', { count: 'exact', head: true })
      ...
      (() => {
        let q = supabase.from('partner_payout_applications').select('expected_commission_amount')
          .eq('partner_type', 'BP')
      ...
      buildStatsQuery('SA_APPROVED'),
      buildStatsQuery('FINANCE_PROCESSING'),
    ])
    ...
    logger.error('Error in BP applications API:', { error })
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (appError || !app) {
      return NextResponse.json({ success: false, error: 'BP payout application not found' }, { status: 404 })
    }
    ...
    switch (action) {
      case 'pick_up':
        if (app.status !== 'PENDING') {
          return NextResponse.json({ success: false, error: `Application is ${app.status}, expected PENDING` }, { status: 409 })
        }
      ...
      case 'verify':
        if (app.status !== 'ACCOUNTS_VERIFICATION') {
          return NextResponse.json({ success: false, error: `Application is ${app.status}, expected ACCOUNTS_VERIFICATION` }, { status: 409 })
        }

        // For direct BP applications, require bank sheet match
        // Team overrides inherit match from linked BA application
        if (!app.is_team_override && !app.bank_sheet_matched) {
          return NextResponse.json({
            success: false,
        ...
        const { data: bpDuplicates } = await supabase
          .from('partner_payout_applications')
        ...
        if (bpDuplicates && bpDuplicates.length > 0) {
          duplicates = bpDuplicates
        }
    ...
    // Update with optimistic lock and row count check
    ...
    if (updateError) {
      logger.error('Error updating BP application:', { error: updateError })
    ...
    if (app.partner_user_id) {
      ...
      if (partnerInfo) {
        notifyPartnerPayoutStatusChange({
          ...
          logger.error('Failed to send BP payout notification:', { error: err, applicationId })
        })
      }
    }
    ...
    logger.error('Error updating BP application:', { error })
    return NextResponse.json({ success: false, error: 'Internal server error' }, { status: 500 })
  }
}
