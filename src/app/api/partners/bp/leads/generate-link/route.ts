 ...
 * POST /api/partners/bp/leads/generate-link
 *
 * Generates a unique, traceable short link for a BP to share with potential customers
 *
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { error: trackingError } = await supabase.from('lead_referral_tracking').insert({
      ...
      shared_by_role: 'BUSINESS_PARTNER',
      shared_by_partner_id: partner.id,
      shared_by_partner_code: partner.partner_id,
      parent_bp_id: null, // BP doesn't have a parent
      parent_bp_code: null,
