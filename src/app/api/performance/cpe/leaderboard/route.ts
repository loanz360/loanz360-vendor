...
/**
 * GET /api/performance/cpe/leaderboard
 * Returns CPE leaderboard ranked by partner revenue
 */
export async function GET(request: NextRequest) {
  try {
    ...
    // Get all CPE employees
    const { data: cpeEmployees, error: employeesError } = await supabase
      .from('users')
    ...
    if (employeesError) {
      apiLogger.error('Error fetching CPE employees', employeesError)
      return NextResponse.json({ success: false, error: 'Failed to fetch employees' }, { status: 500 })
    }

    if (!cpeEmployees || cpeEmployees.length === 0) {
      return NextResponse.json({ leaderboard: [], currentUserRank: 0 })
    }

    // Fetch monthly summaries for all CPEs
    const { data: monthlySummaries } = await supabase
      ...
      .in(
        'user_id',
        cpeEmployees.map((e) => e.id)
      )

    // Create leaderboard entries
    const leaderboardEntries: LeaderboardEntry[] = []

    for (const employee of cpeEmployees) {
      ...
      if (summary) {
        leaderboardEntries.push({
          ...
          primaryMetric: summary.total_partner_revenue || 0,
          secondaryMetric: summary.total_partner_leads_converted || 0,
          tertiaryMetric: summary.partner_conversion_rate || 0,
          trend: 'stable',
          changePercentage: 0,
    ...
    // Sort by primary metric (partner revenue) descending
    ...
    apiLogger.error('Error in CPE leaderboard API', error)
    return NextResponse.json(
