...
function DocCard({ title, description, required, value, error, onUpload, onRemove }: DocCardProps) {
  ...
  const handleFile = async (file: File) => {
    if (!file) return
    try { setUploading(true); await new Promise(r => setTimeout(r, 1000)); onUpload(`https://storage.example.com/publicltd/${Date.now()}-${file.name}`) }
...
export default function DocumentsStep({ data, errors, onUpdate }: PublicLimitedStepProps) {
  const docs = [
    ...
    { key: 'board_resolution_url', title: 'Board Resolution', description: 'Resolution for signatory', required: true },
    { key: 'prospectus_url', title: 'Prospectus', description: 'If listed', required: data.listed_status === 'LISTED' },
    { key: 'sebi_compliance_url', title: 'SEBI Compliance Certificate', description: 'If listed', required: data.listed_status === 'LISTED' },
    { key: 'bank_statement_url', title: 'Bank Statement', description: 'Last 6 months', required: true },
    { key: 'audited_financials_url', title: 'Audited Financials', description: 'Last 2 years', required: true },
