...
import { PublicLimitedStepProps, LISTED_STATUS_OPTIONS, STOCK_EXCHANGE_OPTIONS } from '../../types/public-limited'
import { NATURE_OF_BUSINESS_OPTIONS, GST_STATUS_OPTIONS, ANNUAL_TURNOVER_OPTIONS, ROC_OFFICES } from '../../types'

export default function CompanyDetailsStep({ data, errors, onUpdate }: PublicLimitedStepProps) {
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-2xl font-bold text-white mb-2">Company Details</h2>
        <p className="text-gray-400">Enter the basic information about your Public Limited Company</p>
      </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">Listed Status <span className="text-red-400">*</span></label>
            <select value={data.listed_status} onChange={(e) => onUpdate({ listed_status: e.target.value as typeof data.listed_status })}
              className={`w-full px-4 py-3 bg-gray-800 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 ${errors.listed_status ? 'border-red-500' : 'border-gray-700'}`}>
              <option value="">Select status</option>
              {LISTED_STATUS_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
          </div>

          {data.listed_status === 'LISTED' && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Stock Exchange <span className="text-red-400">*</span></label>
                <select value={data.stock_exchange} onChange={(e) => onUpdate({ stock_exchange: e.target.value })}
                  className={`w-full px-4 py-3 bg-gray-800 border rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-orange-500/50 ${errors.stock_exchange ? 'border-red-500' : 'border-gray-700'}`}>
                  <option value="">Select exchange</option>
                  {STOCK_EXCHANGE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Stock Symbol</label>
                <input type="text" value={data.stock_symbol} onChange={(e) => onUpdate({ stock_symbol: e.target.value.toUpperCase() })} placeholder="e.g., RELIANCE"
