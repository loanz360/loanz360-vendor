...
import { User, ChevronDown, LogOut, HelpCircle, Menu } from 'lucide-react'
...
export function AdminHeader({ onMenuToggle }: { onMenuToggle?: () => void }) {
  ...
  const handleSignOut = async () => {
    const redirectPath = '/admin/auth/login'
    try {
      await signOut()
          ...
          <div className="flex-1 lg:w-[80%] flex justify-between items-center pr-2 lg:pr-6">
            <div className="flex-1 hidden sm:block">
              ...
              <p className="text-gray-400 text-base hidden md:block">
                Manage users, leads, and system operations.
              </p>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  ...
                  <span className="text-base font-medium hidden sm:inline">{user?.full_name || 'Admin'}</span>
                ...
                {showDropdown && (
                  <div className="absolute right-0 mt-2 w-56 bg-gray-900/95 backdrop-blur-lg rounded-lg shadow-xl border border-gray-700/50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="p-3 border-b border-gray-700/50">
                      <p className="text-white text-sm font-medium">{user?.full_name || 'Administrator'}</p>
                      <p className="text-gray-400 text-xs">{user?.email || 'admin@loanz360.com'}</p>
                    </div>

                    <div className="py-2">
                      <button
                        onClick={() => { setShowDropdown(false); router.push('/admin/my-profile') }}
