  ...
  try {
    ...
    if (validRows.length > 0) {
      ...
      if (batchError) {
        ...
        apiLogger.error('BP bulk upload batch insert failed', batchError)
    ...
    return NextResponse.json({
      success: true,
      data: { total: rows.length, success: successCount, failed: errorCount, results },
    })

  } catch (error: unknown) {
    apiLogger.error('BP bulk upload error', error)
