...
export default function DirectorsStepEnhanced({ data, errors, onUpdate }: PrivateLimitedStepProps) {
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid')
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editingMemberIndex, setEditingMemberIndex] = useState<number | null>(null)
  const [numDirectors, setNumDirectors] = useState(data.num_directors || 2)
  ...
  const managingDirectors = filledMembers.filter(m => m.role === 'MANAGING_DIRECTOR')
    ...
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <p className="text-gray-400">
          Add all directors of the company (minimum 2, maximum 15)
        </p>
      </div>
      ...
      <div className="max-w-md mx-auto p-5 bg-gray-800/50 border border-gray-700 rounded-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            max="15"
            value={numDirectors}
            onChange={(e) => handleNumDirectorsChange(parseInt(e.target.value) || 2)}
