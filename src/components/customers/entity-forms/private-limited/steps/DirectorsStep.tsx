...
import { PrivateLimitedStepProps, DirectorData, DIRECTOR_TYPE_OPTIONS, createEmptyDirector } from '../../types/private-limited'
import { GENDER_OPTIONS } from '../../types'

export default function DirectorsStep({ data, errors, onUpdate }: PrivateLimitedStepProps) {
  // Defensive: ensure directors array exists
  const directors = data.directors || []

  const addDirector = () => {
    if (directors.length >= 15) return
    onUpdate({ directors: [...directors, createEmptyDirector()] })
  }
  ...
  const updateDirector = (index: number, updates: Partial<DirectorData>) => {
    ...
    <div className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">Add all directors (Min: 2, Max: 15)</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-1"><Shield className="w-4 h-4 text-orange-400" /><span className="text-sm text-gray-400">With DIN</span></div>
          <p className="text-2xl font-bold text-white">{directors.filter(d => d.din).length}</p>
        </div>
      </div>
          ...
          <div key={director.id} className="bg-gray-800/50 rounded-xl border border-gray-700 p-6">
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center ${director.director_type === 'MANAGING_DIRECTOR' ? 'bg-orange-500/20 text-orange-400' : 'bg-gray-700 text-gray-400'}`}>
              ...
              {directors.length > 2 && <button onClick={() => removeDirector(index)} className="p-2 text-red-400 hover:bg-red-500/10 rounded-lg"><Trash2 className="w-5 h-5" /></button>}
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Director Type</label>
                <select value={director.director_type} onChange={(e) => updateDirector(index, { director_type: e.target.value as DirectorData['director_type'] })}
                  ...
                  {DIRECTOR_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
