...
function DocCard({ title, description, required, value, error, onUpload, onRemove }: DocCardProps) {
  ...
  const handleFile = async (file: File) => {
    if (!file) return
    try { setUploading(true); await new Promise(r => setTimeout(r, 1000)); onUpload(`https://storage.example.com/opc/${Date.now()}-${file.name}`) }
...
export default function DocumentsStep({ data, errors, onUpdate }: OPCStepProps) {
  const docs = [
    ...
    { key: 'company_pan_url', title: 'Company PAN Card', description: 'PAN card of the OPC', required: true },
    { key: 'bank_statement_url', title: 'Bank Statement', description: 'Last 6 months', required: true },
    { key: 'registered_office_proof_url', title: 'Registered Office Proof', description: 'Address proof', required: false },
    { key: 'itr_documents_url', title: 'ITR Documents', description: 'Last 2 years', required: false },
  ...
  return (
    <div className="space-y-8">
      <div><h2 className="text-2xl font-bold text-white mb-2">Documents & KYC</h2><p className="text-gray-400">Upload required company documents</p></div>
      ...
      <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
        <div className="flex items-start gap-3"><AlertCircle className="w-5 h-5 text-yellow-400 flex-shrink-0" /><div><p className="text-yellow-400 font-medium">Director & Nominee Documents</p><p className="text-yellow-300 text-sm">Individual documents for director and nominee can be uploaded in their respective sections.</p></div></div>
      </div>
    </div>
  )
}
